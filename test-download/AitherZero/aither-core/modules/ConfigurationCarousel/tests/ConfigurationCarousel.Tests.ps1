...
<#
.SYNOPSIS
    Comprehensive Pester tests for the ConfigurationCarousel module
...
BeforeAll {
    ...
    $script:ModuleName = "ConfigurationCarousel"
        ...
        It "Should handle errors gracefully" {
            ...
            foreach ($function in $functions) {
                ...
                if ($help -and $help.Parameters) {
                    ...
                    $mandatoryParams = $help.Parameters.Parameter | Where-Object { 
                        $_.Required -eq "true" 
                    }
                    if ($mandatoryParams) {
...
Describe "ConfigurationCarousel Module - Advanced Scenarios" {
    Context "Edge Cases and Boundary Conditions" {
        It "Should handle edge cases properly" {
            ...
            foreach ($function in $functions) {
                ...
                $help = Get-Help $function.Name -ErrorAction SilentlyContinue
                if ($help -and $help.Parameters) {
                    $stringParams = $help.Parameters.Parameter | Where-Object { 
                        $_.Type -and $_.Type.Name -like "*String*" -and $_.Required -eq "false" 
                    }

                    foreach ($param in $stringParams) {
                        if ($param.Name) {
                            # Skip parameters that don't accept empty strings
                            $skipParams = @('Source', 'InputPath', 'BackupName')
                            if ($param.Name -notin $skipParams) {
