...
Describe '0798_generate-changelog' -Tag 'Unit', 'AutomationScript', 'Git Automation & Documentation' {
    ...
    BeforeAll {
        ...
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0798_generate-changelog.ps1'
        $script:ScriptName = '0798_generate-changelog'
    ...
    Context 'Parameters' {
        It 'Should have parameter: FromTag' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('FromTag') | Should -Be $true
        }

        It 'Should have parameter: ToTag' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ToTag') | Should -Be $true
        }

        It 'Should have parameter: Output' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('Output') | Should -Be $true
        }
