...
Describe '0740_Integrate-AITools' -Tag 'Unit', 'AutomationScript', 'AI Integration' {
    ...
    BeforeAll {
        ...
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0740_Integrate-AITools.ps1'
        $script:ScriptName = '0740_Integrate-AITools'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: SkipInstallation' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('SkipInstallation') | Should -Be $true
        }

        It 'Should have parameter: TestConnection' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('TestConnection') | Should -Be $true
        }
