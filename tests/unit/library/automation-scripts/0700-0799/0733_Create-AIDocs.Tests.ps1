...
.DESCRIPTION
    ...
    Stage: Automation
    Description: Generates comment-based help, README files, API documentation, architecture diagrams,
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0733_Create-AIDocs' -Tag 'Unit', 'AutomationScript', 'Automation' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0733_Create-AIDocs.ps1'
