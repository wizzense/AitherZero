...
.DESCRIPTION
    ...
    Stage: Automation
    Description: Analyzes code to generate comprehensive Pester 5.0 tests including unit tests,
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0732_Generate-AITests' -Tag 'Unit', 'AutomationScript', 'Automation' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0732_Generate-AITests.ps1'
