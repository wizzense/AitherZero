...
.DESCRIPTION
    ...
    Stage: Automation
    Description: Performs vulnerability scanning, compliance checking, threat modeling,
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0735_Analyze-AISecurity' -Tag 'Unit', 'AutomationScript', 'Automation' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0735_Analyze-AISecurity.ps1'
