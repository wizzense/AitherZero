...
.DESCRIPTION
    ...
    Stage: Automation
    Description: Creates playbooks, multi-agent task distribution, dependency resolution,
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0736_Generate-AIWorkflow' -Tag 'Unit', 'AutomationScript', 'Automation' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0736_Generate-AIWorkflow.ps1'
