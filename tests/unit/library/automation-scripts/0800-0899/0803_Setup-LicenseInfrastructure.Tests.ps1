...
.DESCRIPTION
    ...
    Script: 0803_Setup-LicenseInfrastructure
    Stage: License Infrastructure
    Description: Configures the complete license infrastructure for an AitherZero organization:
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0803_Setup-LicenseInfrastructure' -Tag 'Unit', 'AutomationScript', 'License Infrastructure' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0803_Setup-LicenseInfrastructure.ps1'
        $script:ScriptName = '0803_Setup-LicenseInfrastructure'
    ...
    Context 'Parameters' {
        It 'Should have parameter: OrganizationName' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('OrganizationName') | Should -Be $true
        }

        It 'Should have parameter: LicenseStoragePath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('LicenseStoragePath') | Should -Be $true
        }
        ...
        It 'Should have parameter: SetupCA' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('SetupCA') | Should -Be $true
        }

        It 'Should have parameter: GenerateMasterKeys' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GenerateMasterKeys') | Should -Be $true
        }

    }

    Context 'Metadata' {
        It 'Should be in stage: License Infrastructure' {
