...
.DESCRIPTION
    ...
    Script: 0870_Create-LicenseStorage
    Stage: License Infrastructure
    Description: Creates the directory structure for storing license files locally.
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:57
#>

Describe '0870_Create-LicenseStorage' -Tag 'Unit', 'AutomationScript', 'License Infrastructure' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0870_Create-LicenseStorage.ps1'
        $script:ScriptName = '0870_Create-LicenseStorage'
    ...
    Context 'Parameters' {
        It 'Should have parameter: StoragePath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('StoragePath') | Should -Be $true
        }
