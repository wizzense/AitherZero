...
.DESCRIPTION
    ...
    Script: 0876_Rotate-LicenseKeys
    Stage: Key Rotation
    Description: Safely rotates encryption keys by:
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:57
#>

Describe '0876_Rotate-LicenseKeys' -Tag 'Unit', 'AutomationScript', 'Key Rotation' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0876_Rotate-LicenseKeys.ps1'
        $script:ScriptName = '0876_Rotate-LicenseKeys'
    ...
    Context 'Parameters' {
        It 'Should have parameter: OldLicensePath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('OldLicensePath') | Should -Be $true
        }

        It 'Should have parameter: NewLicenseId' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('NewLicenseId') | Should -Be $true
        }
        ...
        It 'Should have parameter: EncryptedPattern' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('EncryptedPattern') | Should -Be $true
        }
    ...
    Context 'Metadata' {
        It 'Should be in stage: Key Rotation' {
