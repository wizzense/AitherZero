...
.DESCRIPTION
    ...
    Script: 0802_Load-ObfuscatedModule
    Stage: Runtime
    Description: Transparently decrypts and loads obfuscated PowerShell modules at runtime
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0802_Load-ObfuscatedModule' -Tag 'Unit', 'AutomationScript', 'Runtime' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0802_Load-ObfuscatedModule.ps1'
        $script:ScriptName = '0802_Load-ObfuscatedModule'
    ...
    Context 'Parameters' {
        It 'Should have parameter: EncryptedPath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('EncryptedPath') | Should -Be $true
        }
