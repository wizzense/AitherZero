...
<#
.SYNOPSIS
    Unit tests for 0878_Manage-License
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0878_Manage-License
    Stage: License Management
    Description: Creates, validates, and retrieves licenses for the AitherZero source code
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:57
#>

Describe '0878_Manage-License' -Tag 'Unit', 'AutomationScript', 'License Management' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0878_Manage-License.ps1'
        $script:ScriptName = '0878_Manage-License'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: LicenseId' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('LicenseId') | Should -Be $true
        }

        It 'Should have parameter: LicensedTo' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('LicensedTo') | Should -Be $true
        }

        It 'Should have parameter: ExpirationDays' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ExpirationDays') | Should -Be $true
        }
        ...
        It 'Should have parameter: GitHubOwner' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GitHubOwner') | Should -Be $true
        }

        It 'Should have parameter: GitHubRepo' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GitHubRepo') | Should -Be $true
        }

        It 'Should have parameter: GenerateKey' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GenerateKey') | Should -Be $true
        }

        It 'Should have parameter: SigningKey' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('SigningKey') | Should -Be $true
        }

        It 'Should have parameter: GenerateSigningKey' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GenerateSigningKey') | Should -Be $true
        }

    }

    Context 'Metadata' {
        It 'Should be in stage: License Management' {
