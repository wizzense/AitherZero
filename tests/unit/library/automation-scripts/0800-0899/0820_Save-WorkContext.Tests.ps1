...
.DESCRIPTION
    ...
    Stage: Integration
    Description: Captures complete work state including open files, git status, todos, and variables
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:57
#>

Describe '0820_Save-WorkContext' -Tag 'Unit', 'AutomationScript', 'Integration' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0820_Save-WorkContext.ps1'
