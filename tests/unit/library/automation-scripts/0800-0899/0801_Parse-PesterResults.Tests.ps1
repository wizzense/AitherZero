...
.DESCRIPTION
    ...
    Stage: Testing
    Description: Parses Pester XML/JSON output files to extract test failures,
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0801_Parse-PesterResults' -Tag 'Unit', 'AutomationScript', 'Testing' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0801_Parse-PesterResults.ps1'
