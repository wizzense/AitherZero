...
.DESCRIPTION
    ...
    Stage: Testing
    Description: Runs all test suites without tag filtering
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0409_Run-AllTests' -Tag 'Unit', 'AutomationScript', 'Testing' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0409_Run-AllTests.ps1'
