...
.DESCRIPTION
    ...
    Stage: Testing
    Description: Converts test results to HTML format and publishes them to GitHub Pages
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0450_Publish-TestResults' -Tag 'Unit', 'AutomationScript', 'Testing' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0450_Publish-TestResults.ps1'
