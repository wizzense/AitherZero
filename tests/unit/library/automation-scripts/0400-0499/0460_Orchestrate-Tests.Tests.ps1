...
.DESCRIPTION
    ...
    Script: 0460_Orchestrate-Tests
    Stage: Testing
    Description: Leverages AitherZero's orchestration engine to run tests in the most efficient way:
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:56
#>

Describe '0460_Orchestrate-Tests' -Tag 'Unit', 'AutomationScript', 'Testing' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0460_Orchestrate-Tests.ps1'
        $script:ScriptName = '0460_Orchestrate-Tests'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: ShowProgress' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ShowProgress') | Should -Be $true
        }
