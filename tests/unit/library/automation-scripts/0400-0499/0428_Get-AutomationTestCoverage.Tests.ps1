...
<#
.SYNOPSIS
    Unit tests for 0428_Get-AutomationTestCoverage
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0428_Get-AutomationTestCoverage
    Stage: Testing
    Description: This script provides a comprehensive analysis of test coverage for automation scripts by:
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0428_Get-AutomationTestCoverage' -Tag 'Unit', 'AutomationScript', 'Testing' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0428_Get-AutomationTestCoverage.ps1'
        $script:ScriptName = '0428_Get-AutomationTestCoverage'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: ShowUntested' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ShowUntested') | Should -Be $true
        }

        It 'Should have parameter: ShowTested' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ShowTested') | Should -Be $true
        }

        It 'Should have parameter: Category' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('Category') | Should -Be $true
        }
