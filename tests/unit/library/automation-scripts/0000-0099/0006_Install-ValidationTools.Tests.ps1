...
.DESCRIPTION
    ...
    Script: 0006_Install-ValidationTools
    Stage: Prepare
...
Describe '0006_Install-ValidationTools' -Tag 'Unit', 'AutomationScript', 'Prepare' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0006_Install-ValidationTools.ps1'
        $script:ScriptName = '0006_Install-ValidationTools'
