...
<#
.SYNOPSIS
    Unit tests for 0001_Configure-Environment
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0001_Configure-Environment
    Stage: Environment
    Description: Applies environment configuration settings from config files including:
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:55
#>

Describe '0001_Configure-Environment' -Tag 'Unit', 'AutomationScript', 'Environment' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0001_Configure-Environment.ps1'
        $script:ScriptName = '0001_Configure-Environment'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: GenerateArtifacts' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GenerateArtifacts') | Should -Be $true
        }

        It 'Should have parameter: ArtifactsPath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ArtifactsPath') | Should -Be $true
        }
