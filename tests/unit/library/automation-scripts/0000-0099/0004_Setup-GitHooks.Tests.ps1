...
<#
.SYNOPSIS
    Unit tests for 0004_Setup-GitHooks
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0004_Setup-GitHooks
    Stage: Environment
    Description: Configures Git to use the custom hooks in .githooks/ directory.
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:55
#>

Describe '0004_Setup-GitHooks' -Tag 'Unit', 'AutomationScript', 'Environment' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0004_Setup-GitHooks.ps1'
        $script:ScriptName = '0004_Setup-GitHooks'
