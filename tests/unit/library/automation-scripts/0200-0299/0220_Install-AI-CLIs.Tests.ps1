...
<#
.SYNOPSIS
    Unit tests for 0220_Install-AI-CLIs
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0220_Install-AI-CLIs
    Stage: AI Tools
    Description: Installs command-line interfaces for AI services.
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0220_Install-AI-CLIs' -Tag 'Unit', 'AutomationScript', 'AI Tools' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0220_Install-AI-CLIs.ps1'
        $script:ScriptName = '0220_Install-AI-CLIs'
    ...
    Context 'Parameters' {
        It 'Should have parameter: Tool' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('Tool') | Should -Be $true
        }
    ...
    Context 'Metadata' {
        It 'Should be in stage: AI Tools' {
