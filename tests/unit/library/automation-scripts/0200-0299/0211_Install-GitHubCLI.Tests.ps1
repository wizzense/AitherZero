...
<#
.SYNOPSIS
    Unit tests for 0211_Install-GitHubCLI
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0211_Install-GitHubCLI
    Stage: Development Tools
    Description: Installs the GitHub CLI tool for command-line interaction with GitHub.
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:55
#>

Describe '0211_Install-GitHubCLI' -Tag 'Unit', 'AutomationScript', 'Development Tools' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0211_Install-GitHubCLI.ps1'
        $script:ScriptName = '0211_Install-GitHubCLI'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: Configure' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('Configure') | Should -Be $true
        }

    }

    Context 'Metadata' {
        It 'Should be in stage: Development Tools' {
