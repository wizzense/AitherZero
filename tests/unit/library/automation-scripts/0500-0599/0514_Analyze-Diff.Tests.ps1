...
<#
.SYNOPSIS
    Unit tests for 0514_Analyze-Diff
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0514_Analyze-Diff
    Stage: Reporting
    Description: Compares base and head branches to generate diff statistics, impact analysis,
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0514_Analyze-Diff' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0514_Analyze-Diff.ps1'
        $script:ScriptName = '0514_Analyze-Diff'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: IncludeComplexity' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeComplexity') | Should -Be $true
        }

        It 'Should have parameter: IncludeFunctionLevel' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeFunctionLevel') | Should -Be $true
        }
