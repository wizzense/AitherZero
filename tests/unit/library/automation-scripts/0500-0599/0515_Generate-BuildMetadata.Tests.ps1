...
<#
.SYNOPSIS
    Unit tests for 0515_Generate-BuildMetadata
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0515_Generate-BuildMetadata
    Stage: Reporting
    Description: Creates detailed build-info.json with PR context, git info, environment details
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0515_Generate-BuildMetadata' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0515_Generate-BuildMetadata.ps1'
        $script:ScriptName = '0515_Generate-BuildMetadata'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: IncludePRInfo' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludePRInfo') | Should -Be $true
        }

        It 'Should have parameter: IncludeGitInfo' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeGitInfo') | Should -Be $true
        }

        It 'Should have parameter: IncludeEnvironmentInfo' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeEnvironmentInfo') | Should -Be $true
        }
