...
<#
.SYNOPSIS
    Unit tests for 0513_Generate-Changelog
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0513_Generate-Changelog
    Stage: Reporting
    Description: Analyzes commits between base and head branches, categorizes them (feat/fix/docs),
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0513_Generate-Changelog' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0513_Generate-Changelog.ps1'
        $script:ScriptName = '0513_Generate-Changelog'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: IncludeIssueLinks' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeIssueLinks') | Should -Be $true
        }

        It 'Should have parameter: CategorizeCommits' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('CategorizeCommits') | Should -Be $true
        }
