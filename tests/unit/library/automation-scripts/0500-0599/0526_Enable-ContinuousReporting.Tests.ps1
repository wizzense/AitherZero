...
<#
.SYNOPSIS
    Unit tests for 0526_Enable-ContinuousReporting
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0526_Enable-ContinuousReporting
    Stage: Reporting
...
Describe '0526_Enable-ContinuousReporting' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0526_Enable-ContinuousReporting.ps1'
        $script:ScriptName = '0526_Enable-ContinuousReporting'
