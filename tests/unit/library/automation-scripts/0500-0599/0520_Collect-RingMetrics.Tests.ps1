...
<#
.SYNOPSIS
    Unit tests for 0520_Collect-RingMetrics
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0520_Collect-RingMetrics
    Stage: Reporting
    Description: Gathers metrics for all rings in the ring-based branching strategy including:
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0520_Collect-RingMetrics' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0520_Collect-RingMetrics.ps1'
        $script:ScriptName = '0520_Collect-RingMetrics'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: Rings' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('Rings') | Should -Be $true
        }
