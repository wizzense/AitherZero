...
<#
.SYNOPSIS
    Unit tests for 0518_Generate-Recommendations
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0518_Generate-Recommendations
    Stage: Reporting
    Description: Analyzes all metrics and generates prioritized, actionable recommendations
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0518_Generate-Recommendations' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0518_Generate-Recommendations.ps1'
        $script:ScriptName = '0518_Generate-Recommendations'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: QualityPath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('QualityPath') | Should -Be $true
        }
        ...
        It 'Should have parameter: PrioritizeByImpact' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('PrioritizeByImpact') | Should -Be $true
        }
