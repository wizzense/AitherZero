...
<#
.SYNOPSIS
    Unit tests for 0517_Aggregate-AnalysisResults
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0517_Aggregate-AnalysisResults
    Stage: Reporting
    Description: Combines test, quality, diff, and other analysis results to create a unified
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0517_Aggregate-AnalysisResults' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0517_Aggregate-AnalysisResults.ps1'
        $script:ScriptName = '0517_Aggregate-AnalysisResults'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: IncludeComparison' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeComparison') | Should -Be $true
        }

        It 'Should have parameter: GenerateRecommendations' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GenerateRecommendations') | Should -Be $true
        }
