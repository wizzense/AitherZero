...
<#
.SYNOPSIS
    Unit tests for 0521_Collect-WorkflowHealth
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0521_Collect-WorkflowHealth
    Stage: Reporting
    Description: Collects comprehensive workflow health data including success rates,
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0521_Collect-WorkflowHealth' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0521_Collect-WorkflowHealth.ps1'
        $script:ScriptName = '0521_Collect-WorkflowHealth'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: LookbackDays' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('LookbackDays') | Should -Be $true
        }
