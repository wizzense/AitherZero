...
<#
.SYNOPSIS
    Unit tests for 0524_Collect-QualityMetrics
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0524_Collect-QualityMetrics
    Stage: Reporting
    Description: Aggregates PSScriptAnalyzer results and quality metrics including
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0524_Collect-QualityMetrics' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0524_Collect-QualityMetrics.ps1'
        $script:ScriptName = '0524_Collect-QualityMetrics'
