...
<#
.SYNOPSIS
    Unit tests for 0519_Generate-PRComment
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0519_Generate-PRComment
    Stage: Reporting
    Description: Creates a comprehensive, single PR comment with build info, test results,
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0519_Generate-PRComment' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0519_Generate-PRComment.ps1'
        $script:ScriptName = '0519_Generate-PRComment'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: AnalysisSummaryPath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('AnalysisSummaryPath') | Should -Be $true
        }

        It 'Should have parameter: DashboardPath' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('DashboardPath') | Should -Be $true
        }
        ...
        It 'Should have parameter: IncludeDeploymentInstructions' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeDeploymentInstructions') | Should -Be $true
        }

        It 'Should have parameter: IncludeQuickActions' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeQuickActions') | Should -Be $true
        }
