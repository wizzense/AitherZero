...
<#
.SYNOPSIS
    Unit tests for 0528_Generate-QualityMetrics
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0528_Generate-QualityMetrics
    Stage: Reporting
    Description: Enhances dashboard generation with three-tier validation metrics:
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0528_Generate-QualityMetrics' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0528_Generate-QualityMetrics.ps1'
        $script:ScriptName = '0528_Generate-QualityMetrics'
    ...
    Context 'Parameters' {
        ...
        It 'Should have parameter: MaxHistoryDays' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('MaxHistoryDays') | Should -Be $true
        }
