...
<#
.SYNOPSIS
    Unit tests for 0523_Collect-TestMetrics
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0523_Collect-TestMetrics
    Stage: Reporting
    Description: Collects and aggregates test results from unit, domain, and integration tests.
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0523_Collect-TestMetrics' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0523_Collect-TestMetrics.ps1'
        $script:ScriptName = '0523_Collect-TestMetrics'
