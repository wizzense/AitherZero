...
<#
.SYNOPSIS
    Unit tests for 0527_Generate-CodeMap
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0527_Generate-CodeMap
    Stage: Reporting
...
Describe '0527_Generate-CodeMap' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0527_Generate-CodeMap.ps1'
        $script:ScriptName = '0527_Generate-CodeMap'
