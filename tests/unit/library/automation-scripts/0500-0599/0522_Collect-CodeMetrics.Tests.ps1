...
<#
.SYNOPSIS
    Unit tests for 0522_Collect-CodeMetrics
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0522_Collect-CodeMetrics
    Stage: Reporting
    Description: Analyzes codebase to collect metrics including file counts, lines of code,
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:56
#>

Describe '0522_Collect-CodeMetrics' -Tag 'Unit', 'AutomationScript', 'Reporting' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0522_Collect-CodeMetrics.ps1'
        $script:ScriptName = '0522_Collect-CodeMetrics'
