...
<#
.SYNOPSIS
    Unit tests for 0109_Initialize-InfrastructureSubmodules
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0109_Initialize-InfrastructureSubmodules
    Stage: Infrastructure
    Description: Initializes Git submodules for infrastructure repositories based on configuration in config.psd1.
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:55
#>

Describe '0109_Initialize-InfrastructureSubmodules' -Tag 'Unit', 'AutomationScript', 'Infrastructure' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0109_Initialize-InfrastructureSubmodules.ps1'
        $script:ScriptName = '0109_Initialize-InfrastructureSubmodules'
