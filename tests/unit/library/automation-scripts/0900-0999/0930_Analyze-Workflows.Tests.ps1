...
<#
.SYNOPSIS
    Unit tests for 0930_Analyze-Workflows
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0930_Analyze-Workflows
    Stage: Validation
    Description: Comprehensive analysis of all GitHub Actions workflows including:
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:57
#>

Describe '0930_Analyze-Workflows' -Tag 'Unit', 'AutomationScript', 'Validation' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0930_Analyze-Workflows.ps1'
        $script:ScriptName = '0930_Analyze-Workflows'
    ...
    Context 'Parameters' {
        It 'Should have parameter: ValidateYAML' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ValidateYAML') | Should -Be $true
        }

        It 'Should have parameter: CheckUsage' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('CheckUsage') | Should -Be $true
        }

        It 'Should have parameter: OutputJSON' {
            $cmd = Get-Command $script:ScriptPath
