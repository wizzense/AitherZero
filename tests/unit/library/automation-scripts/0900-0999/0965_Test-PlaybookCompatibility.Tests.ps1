...
.DESCRIPTION
    ...
    Script: 0965_Test-PlaybookCompatibility
    Stage: Testing
    Description: Validates that all three playbook formats (v1.0, v2.0, v3.0) are properly
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:57
#>

Describe '0965_Test-PlaybookCompatibility' -Tag 'Unit', 'AutomationScript', 'Testing' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0965_Test-PlaybookCompatibility.ps1'
        $script:ScriptName = '0965_Test-PlaybookCompatibility'
