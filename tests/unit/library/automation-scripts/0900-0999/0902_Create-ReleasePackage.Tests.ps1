...
<#
.SYNOPSIS
    Unit tests for 0902_Create-ReleasePackage
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0902_Create-ReleasePackage
    Stage: Build
    Description: Creates deployable release packages for AitherZero with configurable options.
    Supports WhatIf: True
    Generated: 2025-11-10 16:41:57
#>

Describe '0902_Create-ReleasePackage' -Tag 'Unit', 'AutomationScript', 'Build' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0902_Create-ReleasePackage.ps1'
        $script:ScriptName = '0902_Create-ReleasePackage'
    ...
    Context 'Parameters' {
        It 'Should have parameter: PackageFormat' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('PackageFormat') | Should -Be $true
        }
        ...
        It 'Should have parameter: IncludeTests' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('IncludeTests') | Should -Be $true
        }

        It 'Should have parameter: OnlyRuntime' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('OnlyRuntime') | Should -Be $true
        }
