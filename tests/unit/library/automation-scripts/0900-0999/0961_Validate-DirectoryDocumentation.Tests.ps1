...
Describe '0961_Validate-DirectoryDocumentation' -Tag 'Unit', 'AutomationScript', '0961 (Documentation Validation)' {
    ...
    BeforeAll {
        ...
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0961_Validate-DirectoryDocumentation.ps1'
    ...
    Context 'Parameters' {
        It 'Should have parameter: CheckMissing' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('CheckMissing') | Should -Be $true
        }

        It 'Should have parameter: GenerateSkeletons' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('GenerateSkeletons') | Should -Be $true
        }

        It 'Should have parameter: ExcludeDirs' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('ExcludeDirs') | Should -Be $true
        }

        It 'Should have parameter: MinFiles' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('MinFiles') | Should -Be $true
        }

    }

    Context 'Metadata' {
        It 'Should be in stage: 0961 (Documentation Validation)' {
