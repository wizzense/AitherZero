...
.DESCRIPTION
    ...
    Script: 0963_Demo-OrchestrationFeatures
    Stage: Demonstration
    Description: This script demonstrates the GitHub Actions-like features added to the orchestration engine:
    Supports WhatIf: False
    Generated: 2025-11-10 16:41:57
#>

Describe '0963_Demo-OrchestrationFeatures' -Tag 'Unit', 'AutomationScript', 'Demonstration' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0963_Demo-OrchestrationFeatures.ps1'
        $script:ScriptName = '0963_Demo-OrchestrationFeatures'
    ...
    Context 'Parameters' {
        It 'Should have parameter: Demo' {
            $cmd = Get-Command $script:ScriptPath
            $cmd.Parameters.ContainsKey('Demo') | Should -Be $true
        }

    }

    Context 'Metadata' {
        It 'Should be in stage: Demonstration' {
