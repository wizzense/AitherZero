...
.DESCRIPTION
    ...
    Script: 0008_Install-OpenTofu
    Stage: Infrastructure
    Description: Install OpenTofu infrastructure as code tool
    Generated: 2025-11-04 20:39:42
#>

Describe '0008_Install-OpenTofu' -Tag 'Unit', 'AutomationScript', 'Infrastructure' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'automation-scripts/0008_Install-OpenTofu.ps1'
        $script:ScriptName = '0008_Install-OpenTofu'
