...
<#
.SYNOPSIS
    Unit tests for 0212_Install-Go
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0212_Install-Go
    Stage: Development Tools
    Description: Installs the Go programming language and sets up GOPATH.
    Supports WhatIf: False
    Generated: 2025-11-07 21:40:54
#>

Describe '0212_Install-Go' -Tag 'Unit', 'AutomationScript', 'Development Tools' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'automation-scripts/0212_Install-Go.ps1'
        $script:ScriptName = '0212_Install-Go'
