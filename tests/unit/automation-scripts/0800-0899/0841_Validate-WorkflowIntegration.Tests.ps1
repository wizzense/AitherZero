...
.DESCRIPTION
    ...
    Script: 0841_Validate-WorkflowIntegration
    Stage: IssueManagement
    Description: Checks for potential conflicts, permission issues, and integration points
    Supports WhatIf: False
    Generated: 2025-11-08 00:28:34
#>

Describe '0841_Validate-WorkflowIntegration' -Tag 'Unit', 'AutomationScript', 'IssueManagement' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'automation-scripts/0841_Validate-WorkflowIntegration.ps1'
        $script:ScriptName = '0841_Validate-WorkflowIntegration'
    ...
    Context 'Metadata' {
        It 'Should be in stage: IssueManagement' {
