BeforeAll {
    ...
    $devEnvironmentPath = Join-Path $projectRoot "$env:PWSH_MODULES_PATH/DevEnvironment"
...
Describe "DevEnvironment Module Tests" {
    Context "Module Import" {
        It "Should import without errors" {
            { Import-Module (Join-Path $projectRoot "$env:PWSH_MODULES_PATH/DevEnvironment") -Force } | Should -Not -Throw
        }
    }
