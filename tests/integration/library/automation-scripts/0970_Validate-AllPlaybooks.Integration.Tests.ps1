...
<#
.SYNOPSIS
    Integration tests for 0970_Validate-AllPlaybooks
.DESCRIPTION
...
Describe '0970_Validate-AllPlaybooks Integration' -Tag 'Integration', 'AutomationScript' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0970_Validate-AllPlaybooks.ps1'
