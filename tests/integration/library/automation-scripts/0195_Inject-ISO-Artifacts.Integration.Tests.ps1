...
<#
.SYNOPSIS
    Integration tests for 0195_Inject-ISO-Artifacts
.DESCRIPTION
...
Describe '0195_Inject-ISO-Artifacts Integration' -Tag 'Integration', 'AutomationScript' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'library/automation-scripts/0195_Inject-ISO-Artifacts.ps1'
    }
