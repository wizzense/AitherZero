...
function Invoke-MonitoredExecution {
    ...
    process {
        try {
            ...
            if (-not $env:CODE_SCANNING_ENABLED) {
                Write-CustomLog 'Code scanning is not enabled. Skipping this step.' -Level WARN
            } else {
                ...
                if ($ErrorHandling -eq 'Comprehensive' -and $CreateIssues) {
                    foreach ($errorRecord in $newErrors) {
                        ...
                        try {
                            ...
                            $errorType = Get-ErrorType -ErrorRecord $errorRecord
            ...
            if ($executionResult.Output -and $CreateIssues -and $ErrorHandling -eq 'Comprehensive') {
                $testFailures = Test-TestFailure -Output $executionResult.Output
