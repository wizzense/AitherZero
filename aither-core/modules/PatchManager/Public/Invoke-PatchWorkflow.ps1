    ...
    process {
        try {
            ...
            if (-not $DryRun) {
                ...
                # NEW: Automatically push the branch for safety and collaboration
                Write-PatchLog 'Pushing branch to remote for safety...' -Level 'INFO'
                git push origin $branchName 2>&1 | Out-Null
                if ($LASTEXITCODE -ne 0) {
                    Write-PatchLog 'Warning: Failed to push branch to remote. Branch exists locally only.' -Level 'WARN'
                    Write-PatchLog "You can push later with: git push origin $branchName" -Level 'INFO'
                } else {
                    Write-PatchLog 'Branch pushed to remote successfully' -Level 'SUCCESS'
                }
            }
            ...
            return @{
                Success        = $true
                BranchName     = $branchName
                BranchPushed   = $true  # Branch automatically pushed for safety
