...
function Resolve-ProjectPath {
    ...
    if (-not $projectRoot) {
        # Find project root by looking for aither-core directory
        $currentPath = $PSScriptRoot
        while ($currentPath -and -not (Test-Path (Join-Path $currentPath "aither-core"))) {
...
function Start-LabAutomation {
    ...
    $useProgressTracking = $ShowProgress -and (Get-Command Start-ProgressOperation -ErrorAction SilentlyContinue)
    ...
    try {
        if ($Steps.Count -gt 0) {
            ...
            if (Get-Command Invoke-ParallelLabRunner -ErrorAction SilentlyContinue) {
                Invoke-ParallelLabRunner -Config $config -ShowProgress:$ShowProgress -ProgressStyle $ProgressStyle
            } else {
                Write-CustomLog -Message "Parallel lab runner not available, using sequential execution" -Level "WARN"
            }
...
function Start-EnhancedLabDeployment {
    ...
    try {
        ...
        $hasOpenTofuProvider = Get-Command Start-InfrastructureDeployment -ErrorAction SilentlyContinue
