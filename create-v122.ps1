...
Invoke-PatchWorkflow -PatchDescription "Release v1.2.2 - Working Quickstart with Enhanced Setup Wizard" -PatchOperation {
    # Update VERSION file
    $versionContent = Get-Content "VERSION" -Raw
    $newContent = $versionContent -replace '1\.2\.1', '1.2.2'
...
"@
    Set-Content "RELEASE-NOTES-1.2.2.md" -Value $releaseNotes -NoNewline
