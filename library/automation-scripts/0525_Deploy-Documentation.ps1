...
function Write-ScriptLog {
    ...
    if (Get-Command Write-CustomLog -ErrorAction SilentlyContinue) {
        Write-CustomLog -Level $Level -Message $Message -Source "0525_Deploy-Documentation" -Data $Data
    } else {
...
function Copy-ExistingReports {
    ...
    $reportSources = @(
        ...
        @{ Source = Join-Path $ProjectPath "library/tests/reports"; Destination = Join-Path $OutputPath "reports/latest" }
