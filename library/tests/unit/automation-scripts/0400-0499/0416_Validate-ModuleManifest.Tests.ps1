...
<#
.SYNOPSIS
    Unit tests for 0416_Validate-ModuleManifest
.DESCRIPTION
    Auto-generated comprehensive tests with environment awareness
    Script: 0416_Validate-ModuleManifest
    Stage: Testing
    Description: This script validates .psd1 module manifest files for:
    Supports WhatIf: False
    Generated: 2025-11-08 00:28:33
#>

Describe '0416_Validate-ModuleManifest' -Tag 'Unit', 'AutomationScript', 'Testing' {

    BeforeAll {
        # Compute path relative to repository root using $PSScriptRoot
        $repoRoot = Split-Path (Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent) -Parent
        $script:ScriptPath = Join-Path $repoRoot 'automation-scripts/0416_Validate-ModuleManifest.ps1'
        $script:ScriptName = '0416_Validate-ModuleManifest'
